...
# --- 2. ميزات V8.0 المتقدمة (Shannon Entropy & Specific Counts) ---

def calculate_entropy(text):
    """حساب العشوائية في النص لاكتشاف الحمولات المشفرة أو المشبوهة"""
...
def extract_features_v8(text):
    ...
    # الرموز الخاصة والكلمات المفتاحية (توسيع من V7.5)
    ...
    return {
        ...
        'entropy': calculate_entropy(text),        # ميزة جديدة لتمييز التشفير
        'semicolon_count': text.count(';'),       # ميزة جديدة لاكتشاف SQL Injection
        'apostrophe_count': text.count("'"),      # ميزة جديدة لاكتشاف كسر السلاسل النصية
        'bracket_count': text.count('(') + text.count(')') # ميزة جديدة لاكتشاف الدوال البرمجية
    }
...
# زيادة حجم البيانات لضمان تعلم عميق
...
iot_X = pd.DataFrame()
# محاكاة الميزات للبيانات التي لا تحتوي على نصوص مباشرة
...
iot_X['entropy'] = (df_iot['AVG'] / 100).fillna(0) # ميزة بديلة للبيانات الرقمية
iot_X['semicolon_count'] = 0
iot_X['apostrophe_count'] = 0
iot_X['bracket_count'] = 0
iot_y = df_iot['label']

# --- 4. الدمج والموازنة الذكية ---
...
# زيادة العينات لتعزيز الدقة (100 ألف لكل فئة)
n_samples = 100000
...
# --- 5. التدريب الفائق (Hyper-Tuned Random Forest) ---
print(f"🧠 Training Ultra-Precision Model on {len(X)} samples with 10 Features...")
# رفعنا n_estimators لـ 500 و max_depth لـ 35 للوصول لأقصى دقة
